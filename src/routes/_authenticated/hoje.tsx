...
function Hoje() {
  ...
  return <AppShell pacientes={pacientes} pacienteId={pacienteId} onTrocarPaciente={selecionar}>
    ...
    {proxima ? <section className="rounded-lg bg-primary p-5 text-primary-foreground"><div className="flex items-center justify-between gap-3"><span className="rounded-md bg-card/15 px-3 py-1 text-sm font-bold">{proxima.status === "atrasado" ? "Dose atrasada" : "Próxima dose"}</span><span className="font-display text-2xl font-semibold">{proxima.hora}</span></div><p className="mt-4 font-display text-3xl font-semibold">{proxima.medication.nome}</p><p className="mt-1 text-lg text-primary-foreground/90">{proxima.medication.dosagem}{proxima.medication.instrucoes ? ` · ${proxima.medication.instrucoes}` : ""}</p><Button onClick={() => marcar.mutate(proxima)} disabled={marcar.isPending} variant="secondary" size="lg" className="mt-5 w-full text-primary">Registrar como tomada</Button></section> : <section className="surface"><p className="section-heading">{meds.length === 0 ? "Nenhum medicamento cadastrado" : "Doses concluídas hoje"}</p><p className="mt-2 text-inksoft">{meds.length === 0 ? "Use o assistente para cadastrar o primeiro medicamento." : "Todas as doses previstas para hoje foram registradas."}</p></section>}
    ...
    <section className="surface"><div className="flex items-center gap-3"><MessageCircleHeart className="size-5 text-primary" /><h2 className="section-heading">Assistente de medicamentos</h2></div><p className="mt-2 text-inksoft">Informe nome, dosagem, intervalo e duração para organizar o medicamento.</p><Button asChild className="mt-4 w-full"><Link to="/conversar"><Plus />Registrar medicamento</Link></Button></section>

    {meds.length > 0 ? <section><h2 className="section-heading mb-3">Tratamentos ativos</h2><div className="divide-y divide-border rounded-lg bg-card px-4 ring-1 ring-border">{meds.map((med) => <div key={med.id} className="py-3"><p className="font-display text-lg font-semibold">{med.nome}</p><p className="text-base text-inksoft">{med.dosagem} · {descricaoFrequencia(med)}</p></div>)}</div></section> : null}
  </AppShell>;
}
