        ...
        {temFotoSelecionada ? (
          <>
            ...
            <div className="mt-4">
              ...
              <p className="mt-1 text-base text-inksoft">Foto registrada no acompanhamento de hoje.</p>
            </div>
          </>
        ) : (
          <>
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-inksoft">Registro diário</p>
                <h2 className="section-heading">{diaSelecionado} de setembro de 2026</h2>
              </div>
            </div>
            <p className="mt-3 text-base text-inksoft">Sem foto registrada para este dia.</p>
          </>
        )}
      </section>
    </section>
...
function DiarioConvidado() {
  const [diaSelecionado, setDiaSelecionado] = useState(17);
  const dias = Array.from({ length: 30 }, (_, i) => i + 1);
  const fotoDia17 = FOTO_JOSE_17_09;
      ...
      <section className="space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-bold text-inksoft">Acompanhamento AICare</p>
            <h1 className="page-heading">Diário · Setembro 2026</h1>
            <p className="mt-1 text-base text-inksoft">Fotos e registros diários de José</p>
          </div>
          <Camera className="size-6 text-primary" />
        </div>

        <div className="mt-4 flex items-center justify-between rounded-lg bg-card p-2 ring-1 ring-border">
          <button type="button" aria-label="Mês anterior" className="grid size-10 place-items-center rounded-md text-inksoft hover:bg-chrome-tint"><ChevronLeft /></button>
          <span className="font-bold">Acompanhamento diário</span>
          <button type="button" aria-label="Próximo mês" className="grid size-10 place-items-center rounded-md text-inksoft hover:bg-chrome-tint"><ChevronRight /></button>
        </div>
        ...
        <div className="grid grid-cols-7 gap-1 rounded-lg bg-card p-3 ring-1 ring-border">
          ...
          {dias.map((dia) => {
            ...
            const selecionado = diaSelecionado === dia;
            return (
              <button key={dia} type="button" onClick={() => setDiaSelecionado(dia)} aria-label={temFoto ? `Dia ${dia}, com foto` : `Dia ${dia}, sem foto registrada`} aria-pressed={selecionado} className={`min-h-16 rounded-md p-1 text-center transition-colors focus:outline-none focus:ring-2 focus:ring-ring sm:min-h-20 ${selecionado ? "bg-chrome-tint ring-2 ring-primary" : "hover:bg-chrome-tint/60"}`}>
                {temFoto ? <div className="overflow-hidden rounded-md bg-chrome-tint"><img src={fotoDia17} alt="" className="h-11 w-full object-cover sm:h-14" /></div> : <div className="grid h-11 place-items-center text-base font-semibold sm:h-14">{dia}</div>}
                <span className="mt-1 block text-[10px] font-bold text-inksoft">{dia}</span>
              </button>
            );
          })}
        ...
        <section className="surface">
          <div className="overflow-hidden rounded-lg bg-chrome-tint"><img src={fotoDia17} alt="Registro diário de José em 17 de setembro de 2026" className="max-h-80 w-full object-cover" /></div>
          <div className="mt-4">
            <p className="text-sm font-bold text-inksoft">Registro diário</p>
            <h2 className="section-heading">17 de setembro de 2026</h2>
            <p className="mt-1 text-base text-inksoft">Foto registrada no acompanhamento de hoje.</p>
