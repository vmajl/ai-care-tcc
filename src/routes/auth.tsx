...
function AuthPage() {
  ...
  return <div className="min-h-screen bg-canvas font-body text-lg text-ink"><main className="mx-auto max-w-[460px] space-y-6 px-5 py-8">
    <Link to="/" className="flex items-center gap-3"><span className="chrome grid size-12 place-items-center rounded-xl ring-1 ring-on-chrome/60"><Bell className="size-6 text-on-chrome" strokeWidth={2.5} /></span><span className="font-display text-2xl font-semibold">AICare</span></Link>
    {confirmar ? <section className="rounded-xl bg-card p-6 ring-1 ring-border"><h1 className="font-display text-3xl font-semibold">Confira seu e-mail</h1><p className="mt-3 text-inksoft">Enviamos um link de confirmação para <strong>{email}</strong>. Abra o e-mail e toque no link para entrar.</p></section> : <section className="rounded-xl bg-card p-6 ring-1 ring-border">
      ...
      {modoConvidado ? <div className="mt-3 rounded-xl bg-chrome-tint p-4 text-base text-inksoft ring-1 ring-border"><div className="flex items-center gap-2 font-bold text-chrome-deep"><UserPlus className="size-5" />Convite: {codigoConvite || "código recebido"}</div><p className="mt-1">Crie sua conta para acompanhar os registros compartilhados pelo cuidador.</p></div> : null}
      <form onSubmit={enviar} className="mt-5 space-y-4">
        ...
        <button type="submit" disabled={carregando} className="chrome w-full rounded-xl py-5 font-display text-2xl font-bold text-on-chrome ring-1 ring-on-chrome/50 transition-transform hover:scale-[1.02] active:scale-95 disabled:opacity-70">{carregando ? "Aguarde…" : modo === "entrar" ? "Entrar" : "Criar conta"}</button>
      </form>
      {!modoConvidado ? <><button onClick={entrarComGoogle} className="mt-3 w-full rounded-xl bg-chrome-tint py-4 font-display text-xl font-bold text-chrome-deep ring-1 ring-border">Entrar com o Google</button><Link to="/convite" className="mt-3 block w-full rounded-xl bg-chrome-tint py-4 text-center font-display text-xl font-bold text-chrome-deep ring-1 ring-border">Entrar como convidado</Link><button onClick={() => setModo(modo === "entrar" ? "criar" : "entrar")} className="mt-5 w-full text-center text-base font-bold text-inksoft underline">{modo === "entrar" ? "Ainda não tenho conta" : "Já tenho uma conta"}</button></> : null}
    </section>}
  </main></div>;
}
